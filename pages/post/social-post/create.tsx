...
const CreatePostHome: NextPage = () => {
  ...
  const [removeErroringFileFlag, setRemoveErroringFileFlag] = useState<number>(0);
  ...
  const valuingMediaForUpdate = () => {
    ...
    post.mediaUrls.map((value) => {
      newMedia.push({ link: value.url, type: value.isVideo ? 'video' : 'image', uniqueId: uuidv4(), previewLink: '' });
    });
