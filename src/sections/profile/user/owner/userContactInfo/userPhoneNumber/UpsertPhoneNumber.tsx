...
function AddPhoneNumber() {
  ...
  useEffect(() => {
    if (!userPhoneNumber) router.push('/profile/user/contact-info/list');
  ...
  const onSubmit = async (data: UserPhoneNumberType) => {
    ...
    if (resData.data?.upsertPhoneNumber?.isSuccess) {
      ...
      router.push('/profile/user/contact-info/phone-number/verify-phone-number');
    }
  ...
  const handleDialogDeletePhoneNumber = () => {
    router.push('/profile/confirm-delete-number');
  };
