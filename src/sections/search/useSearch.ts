...
import { getConfirmSearch, setActiveFilter, setCount, setLoading, setValues, updateValues } from 'src/redux/slices/search';
import { useDispatch, useSelector } from 'src/redux/store';
import { useLazyGetFundRaisingPostsQuery } from 'src/_requests/graphql/post/getCampaignPosts.generated';
...
const useSearch = (
  ...
  const filters = useSelector(getConfirmSearch);
  ...
  useEffect(() => {
    if (canSendRequest && changeFilterFlag) {
      needToReset.current = true;
      sendRequest(true);
