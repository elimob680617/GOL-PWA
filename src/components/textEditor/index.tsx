...
import { ERROR, PRIMARY, SURFACE } from 'src/theme/palette';
import Image from 'next/image';
import { basicSharePostSelector, setSharedPostText } from 'src/redux/slices/post/sharePost';
import { basicSendPostSelector, setSendPostText } from 'src/redux/slices/post/sendPost';
...
const MentionExample = (props: any) => {
  const { setListOfRichs, media, setTextLimitation, placeholder, eventType } = props;
  ...
  const slateInitialValue: Descendant[] =
    eventType === 'createPost' ? post.text : eventType === 'sharePost' ? postShared.text : postSent.text;
  ...
  const createHashtag = () => {
    ...
    createTagMutation({
      ...
      .then((res) => {
        ...
        insertTag(editor, res.createTag.listDto.items[0].title!, res.createTag.listDto.items[0].id!);
        setHashtagTarget(null);
      })
      .catch((err) => {
        console.log(err);
  ...
  const handleTextLimit = (value: any) => {
    let postText = '';
    value.map((item) => {
      item.children &&
        item?.children.map &&
    ...
    setTextLength(postText.length - 1);
    setTextLimitation(postText.length - 1);
  };
  return (
    <Slate
      editor={editor}
      ...
      onChange={(value) => {
        ...
        // const deserializeToHtml = convertToHtmlForUpdate(value);
        // const document = new DOMParser().parseFromString(deserializeToHtml, 'text/html');
        // let deserilize = deserialize(document.body);
        // deserilize = removeIncorrectNodes(deserilize);
        setSlateValue(value);
        eventType === 'createPost' && textLength > 3000 ? dispatch(setText(textValue)) : dispatch(setText(value));
        eventType === 'sharePost' && textLength > 3000
          ? dispatch(setSharedPostText(textValue))
          : dispatch(setSharedPostText(value));
        eventType === 'sendPost' && textLength > 1000
          ? dispatch(setSendPostText(textValue))
          : dispatch(setSendPostText(value));
        dispatch(setText(value));
        setListOfRichs(value);
        handleTextLimit(value);
      ...
      <Typography variant="subtitle1">
        <Typography variant="body1" color={'text/disabled'}>
          ...
          <Editable
            ...
            placeholder={placeholder ? placeholder : 'What do you want to talk about?'}
            aria-disabled={true}
            disabled={true}
            readOnly={textLength >= 3000 ? true : false}
            style={media?.length === 0 ? { width: '100%', height: '19rem', zIndex: 1 } : { width: '100%', zIndex: 1 }}
          />
        </Typography>
